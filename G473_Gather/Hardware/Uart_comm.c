...
/**
  * @brief  发送电压、电流、温度结构体（打包为帧格式）
  * @param  tx_struct: 待发送的传感器数据结构体指针
  */
void UART1_Send_Struct(UART_TxStruct *tx_struct)
{
  ...
  const uint16_t total_size = header_size + struct_size + footer_size;
...
{
  static uint8_t parse_state = 0;  // 0:等待帧头, 1:接收结构体, 2:等待帧尾
  ...
  while (uart1_dev.rx_buf.head != uart1_dev.rx_buf.tail)
    {
      ...
      switch (parse_state)
        ...
        case 0:  // 等待帧头0xAA
          if (data == 0xAA)
            {
              uart1_dev.rx_parse_len = 0;  // 重置解析长度
              parse_state = 1;             // 进入接收结构体状态
            }
          break;

        case 1:  // 接收UART_RxStruct结构体（4字节：启动标志+模式+预留字节）
          ...
          if (data == 0xAA)
            {
              uart1_dev.rx_parse_len = 1;  // 当前字节作为新帧头
              parse_state = 1;
            }
          break;

        case 2:  // 等待帧尾0x55
          if (data == 0x55)    // 帧尾正确，更新全局数据
            {
              __disable_irq();
              // 复制解析后的结构体（控制指令）到设备实例
              memcpy(&uart1_dev.rx_data, uart1_dev.rx_parse_buf, sizeof(UART_RxStruct));
              // 同步到全局变量，供其他模块读取
              uart_rx_data = uart1_dev.rx_data;
              __enable_irq();
            }
          parse_state = 0;  // 无论帧尾是否正确，重置状态
          break;
        }
    }
}
...
{
  MX_USART1_UART_Init();   // 初始化硬件
  InitHardUart1();         // 初始化中断
  Uart1VarInit();          // 初始化变量
}

/**
  * @brief  UART1中断服务函数（处理接收和发送）
  */
