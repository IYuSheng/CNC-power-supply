...
//  // 非阻塞轮询：等待转换完成（OS位清零）
//  uint32_t timeout = 5000;  // 超时计数（约5ms，远大于1.25ms转换时间）
//  uint16_t config_status;
//  do
//    {
//      // 读取配置寄存器，检查OS位状态
//      status = I2C_Read16(I2Cx, SGM58031_I2C_ADDR, SGM58031_REG_CONFIG, &config_status);
//      if(status != ADC_OK) return status;  // I2C读取失败

//      // 若OS位为0，说明转换完成
//      if((config_status & SGM58031_CONFIG_OS_SINGLE) == 0)
//        {
//          break;
//        }
//    }
//  while(timeout-- > 0);    // 超时退出

//  if(timeout == 0) return ADC_TIMEOUT;  // 转换超时
...
ADC_StatusTypeDef SGM58031_ReadVoltage(I2C_TypeDef *I2Cx, uint8_t channel, float *voltage)
{
  ...
  // 读取原始ADC值
  status = SGM58031_ReadChannel(I2Cx, channel, &adc_raw);
  if(status != ADC_OK)
    {
      return status;
    }
  ...
  *voltage = SGM58031_ConvertToVoltage_4096V(adc_raw);
  ...
  Debug_printf("Channel %d: Raw=0x%04X (%d), Voltage=%.4fV",
               channel, (uint16_t)adc_raw, adc_raw, *voltage);
